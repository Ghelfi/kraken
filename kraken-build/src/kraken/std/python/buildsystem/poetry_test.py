...
from kraken.std.python.pyproject import PackageIndex, TomlConfig
...
def test__PoetryPyprojectHandler__getters() -> None:
    handler = PoetryPyprojectHandler(TomlConfig.read_string(EXAMPLE_POETRY_PYPROJECT))
...
def test__PoetryPyprojectHandler__get_package_indexes__with_legacy_source_config() -> None:
    handler = PoetryPyprojectHandler(TomlConfig.read_string(EXAMPLE_POETRY_PYPROJECT_WITH_LEGACY_SOURCE_CONFIG))
    assert handler.get_package_indexes() == [
        PackageIndex(
            alias="foo",
            index_url="https://foo.bar/simple/",
